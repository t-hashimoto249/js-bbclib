...
//    let bbcAsset = await helper.create_asset(user_id);
    ...
    bbcRelation.add_pointer(new BBcPointer(transaction_id, asset_id));
  ...
  it('Test for BBcSignature Class.', async function () {
  ...
  it('Test for BBcTransaction Class.', async () => {
    ...
    let transaction_serialize = await bbctransaction.serialize(false);
    ...
    for (let i = 0; i < bbctransaction.relations.length; i++) {
      let serialize_relations = bbctransaction.relations[i];
      let deserialize_relations = transaction_deserialize.relations[i];
    ...
    for (let i = 0; i < bbctransaction.witness.sig_indices.length; i++) {
      let serialize_witness = bbctransaction.witness;
      let deserialize_witness = transaction_deserialize.witness;
